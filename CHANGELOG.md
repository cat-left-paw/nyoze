...
現行の GitHub Latest と Microsoft Store版は、いずれも `0.4.0-beta.2` です。Microsoft Store package versionは`1.2.4.0`です。

## 0.4.0-beta.2

### 不具合修正

- File Explorerで選択中のフォルダまたはファイルの親フォルダではなく、書庫のrootへ新規フォルダが作成される問題を修正
- File Explorerで文書やフォルダをゴミ箱へ移動したあと、Windowsで本文の文字入力ができなくなる問題を修正
- 開いている文書またはその親フォルダをゴミ箱へ移動しても、対象文書のタブが残り編集できてしまう問題を修正
- 既存文書を開いた直後に本文へfocusが入らず、追加クリックなしでは入力できない場合がある問題を修正
- 文書冒頭のメタデータ表示中に文書を開くと、擬似キャレットが本文ではなくタイトル先頭へ表示される問題を修正
- 文書切り替え直後に、文書情報のタイトルなどがクリックするまで更新されない場合がある問題を修正

### 主な追加・改善

- File Explorerで新規文書を作成したとき、その文書を新しいタブで開き、そのまま入力できるように改善
- 空の文書や短い文書で、本文より後ろの余白をクリックすると文書末尾へキャレットを置けるように改善
- 終了時に開いていた保存済み文書のタブ、順序、アクティブなタブを次回起動時に復元
- Windowsの表示設定へ **Chromiumアクセシビリティを無効化する** を追加。一部の環境でDeepLなどの常駐アプリと同時利用した際に著しく遅くなる場合の回避策として利用可能
- Microsoft Store向けのタイル・ロゴ画像を現在のアプリアイコンへ更新
