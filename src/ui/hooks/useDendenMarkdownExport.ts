...
export function useDendenMarkdownExport({
  coreRef,
  activeTab,
  internalDocActive,
  fullPlainEditActive,
  ...
  const exportActiveDocument = useCallback(async () => {
    ...
    if (fullPlainEditActive || paragraphPlainModeActive) {
      showEditorInlineHint(getUiText(uiLanguageMode, 'export.dendenRejectedPlainMode'))
      return
    }

    const targetTabId = activeTab.id
    const targetFilePath = activeTab.filePath
    const selection = await requestExportOptions('denden')
    if (!selection) return
    ...
    const exported = coreRef.current?.exportDendenMarkdown({
    ...
    const bridge = window.nyozeBridge?.fs?.exportDendenMarkdown
    if (!bridge) return

    const suggestedPath = suggestDendenExportPath(activeTab.filePath)
    let retry = true
      ...
      if (result.saved) {
        ...
        showGlobalNotice(
          exported.warnings.length > 0
            ? getUiText(uiLanguageMode, 'export.dendenSuccessWithWarnings')
            : getUiText(uiLanguageMode, 'export.dendenSuccess'),
        )
        return
      }
