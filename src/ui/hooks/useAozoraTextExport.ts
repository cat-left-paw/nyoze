...
export function useAozoraTextExport({
  coreRef,
  activeTab,
  internalDocActive,
  fullPlainEditActive,
  ...
  const exportActiveDocument = useCallback(async () => {
    ...
    if (fullPlainEditActive || paragraphPlainModeActive) {
      showEditorInlineHint(getUiText(uiLanguageMode, 'export.aozoraPlainModeBlocked'))
      return
    }

    const targetTabId = activeTab.id
    const targetFilePath = activeTab.filePath
    const selection = await requestExportOptions('aozora')
    if (!selection) return
    ...
    const exported = coreRef.current?.exportAozoraText({
    ...
    const bridge = window.nyozeBridge?.fs?.exportAozoraText
    if (!bridge) return

    const suggestedPath = suggestAozoraExportPath(activeTab.filePath)
    let retry = true
      ...
      if (result.saved) {
        ...
        showGlobalNotice(
          exported.warnings.length > 0
            ? getUiText(uiLanguageMode, 'export.aozoraSuccessWithWarnings')
            : getUiText(uiLanguageMode, 'export.aozoraSuccess'),
        )
        return
      }
