...
- `Nyoze-Mac-0.4.0-beta.2-arm64-Installer.dmg`
...
- 終了時に開いていた保存済み文書のタブを、順序とアクティブなタブを含めて次回起動時に復元します。未保存の新規文書、未保存の本文、キャレット・選択・スクロール位置は復元対象に含みません。復元対象のファイルが見つからない場合は、そのタブだけを読み飛ばします。
- File Explorerで新規文書を作成すると、その文書を新しいタブで開き、本文先頭からそのまま入力できるようにしました。
- 空の文書や短い文書では、本文より後ろの余白をクリックして文書末尾へキャレットを置けるようにしました。
- Windows版の表示設定へ **Windows互換性 > Chromiumアクセシビリティを無効化する** を追加しました。一部の環境でDeepLなどの常駐アプリと同時に使った際、編集が著しく遅くなる場合の回避策です。設定変更後はNyozeの再起動が必要です。
- Microsoft Store向けのタイル・ロゴ画像を、現在のアプリアイコンへ更新しました。

### 主な修正

- File Explorer上部の新規フォルダ作成で、選択中のフォルダまたはファイルの親フォルダではなく、書庫のrootへ作成される問題を修正しました。
- File Explorerで文書やフォルダをゴミ箱へ移動したあと、Windowsで本文の文字入力ができなくなる問題を修正しました。削除を確定した場合とキャンセルした場合のどちらでも、本文へ戻って入力を続けられます。
- 開いている文書またはその親フォルダをゴミ箱へ移動した場合、削除された文書のタブを閉じるようにしました。
- 新規・既存文書を開いた直後の本文focusとキャレットを揃え、追加クリックなしで入力できるようにしました。
- 文書冒頭のメタデータ表示中に文書を開いたとき、擬似キャレットがタイトル先頭へずれて表示される問題を修正しました。
- 文書切り替え直後に、左ペインの文書情報へタイトルなどがすぐ反映されない場合がある問題を修正しました。

### 更新前の注意

- 更新前にNyozeを終了し、重要な原稿と作品フォルダは別途バックアップしてください。
- 起動時のタブ復元は、ディスクに保存済みの通常文書だけを対象とします。未保存内容の復旧機能ではありません。
- **Chromiumアクセシビリティを無効化する** は既定OFFです。スクリーンリーダーなどの支援技術を利用している場合は有効にしないでください。

### 現時点の既知制限

- 長文編集モードは**実験的・既定OFF**です。未対応位置や安全に引き継げない操作では待機中になり、通常エディタへ戻ります。
- 長文編集モードで局所編集面が「編集中」の間は、タイプライターモードなど一部の表示・編集補助機能を同時に利用できません。
- 長文編集モードを安全に維持できない場合、未確定の編集内容を保護したまま「回復（recovery）」状態になることがあります。macOS / Windows実機ではこの状態を自然に再現できなかったため、回復状態での実機確認は未了です。
- Linuxではソース版・ローカルbuildから長文編集モードを試せますが、動作保証、公式サポート、公式package配布の対象外です。
- ルビや明示TCYの直後で日本語IME入力を始めたとき、環境やタイミングによっては、まれに入力が詰まることがあります。その場合は`Escape`で未確定入力を破棄して通常編集へ戻れます。
- 大きな文書では、文書の内容・構造や端末環境によって、切り替え直後の操作が重くなる場合があります。長文編集モードまたはルビ表示をOFFにする、`Paragraph Plain`を使う、章単位でファイルを分ける運用を検討してください。
