...
- `Nyoze-Windows-0.4.0-beta.1-x64.zip`
...
- **長文編集モード（実験的）**を追加しました。既定はOFFで、ツールバーまたは表示設定から明示的に有効にできます。長い文書の対応位置では局所的な編集面を使い、ルビ / 縦中横の境界や範囲選択・構造操作では通常エディタへ安全に戻ります。macOS / Windowsを確認対象とし、Linuxでは非サポートの試験利用としてソース版・ローカルbuildから利用できます。
- 長文編集モードの **オフ / 待機中 / 編集中** を左下の状態アイコンで確認できるようにし、表示設定とツールバーの名称を統一しました。書字方向切替、同一タブ内のファイル切替、WindowsのPageUp / PageDown後にも、条件が整えば追加クリックなしで再開します。
- メインウィンドウの通常時のサイズ・位置と最大化状態を保存し、次回起動時に利用可能な画面内へ復元するようにしました。初期サイズと最小サイズも、タスクバーを含む実際のwork areaへ収まるように調整します。
- 狭いウィンドウで上部ツールバーが隠れる場合、ホイール、下端のつまみ、専用グリップで左右へ移動できるようにしました。ウィンドウ移動用グリップとツールバー移動用グリップは別の操作です。
- 左右ペインの開閉を軽くし、開閉後に擬似キャレットがずれにくいようにしました。
- 横書き本文の折り返し行末をjustifyし、長文編集モード中も章頭・章末のナビゲーションが隠れないようにしました。見出し、code block、最終行は不自然に引き伸ばしません。
- 付箋の段落末マーカーが余分な1行・1列を作る表示と、画面上端付近でhover previewが欠ける問題を修正しました。付箋を追加して本文を保存せず破棄した場合は、Markdown markerと未保存の付箋内容を一緒に破棄します。

### 主な修正

- 長文・ルビ多用文書で、通常入力のたびにルビ直後約物の表示補正が全文を再走査していた処理を局所化しました。
- 長文編集モードの開始・pointer handoff時に、表示予約pluginの付け直しによってルビ表示用DOMを不要に再走査していた処理を撤去しました。これは不要な処理を除く修正であり、巨大文書全体の操作速度を保証するものではありません。
- macOSで未保存文書の確認後に「保存して終了」または「破棄して終了」を選んでも、ウィンドウだけが消えてアプリが終了しない問題を修正しました。赤いclose buttonによる通常のウィンドウcloseは従来どおりです。
- 同一段落内に「ルビ＋直後の句読点」と後続のルビ / 縦中横が共存するとき、後続境界で日本語入力が詰まりやすい問題を限定的に緩和しました。
- 付箋の削除・整理・ファイル移動時に、`notes.json`のタグ定義など付箋本文以外の情報が失われ得る問題を修正しました。
...
`0.3.0-beta.1` 公開後に確認された不具合を修正するpre-releaseです。Book / Project、Page Viewer、Web Book、外部exportの機能範囲は広げず、安定化と案内の修正だけを行いました。

### 0.3.0-beta.1からの修正

- 章境界ナビゲーションが無効な状態で renderer の再描画が繰り返され、文書や書庫を開いていないときにも CPU 使用率が高止まりする問題を修正しました。
- 見出しマーカーの直後に全角空白を入れた見出しで、再解析や保存を繰り返すたびに全角空白が増える問題を修正しました。
- エディタのタブ列右端にある **検索**、**ルビ表示**、**段落プレーン編集**、**ソースモード**、**タイプライター**、**表示設定**、**Page Viewer** の tooltip が表示領域から隠れる問題を修正しました。
- 書庫が未登録のときの案内を、現在利用できる「新しい書庫を作成」「既存フォルダを書庫として登録」の操作に合わせて更新しました。

### 更新前の注意

- `0.3.0-beta.1` を利用している場合は、Nyoze を終了してから `0.3.0-beta.2` へ更新してください。
- 重要な原稿と作品フォルダは、更新前に別途バックアップしてください。
- Page Viewer の Book 全体表示と Book 全体 export は、ディスク上の保存済み章を読み取ります。未保存の編集内容は含みません。
- Web 公開用 package は、新規または空のフォルダにだけ作成できます。既存 package の上書き、更新、差分同期は未対応です。
- Microsoft Store 版には、`0.3.0-beta.1` / `0.3.0-beta.2` の新機能と修正はまだ含まれません。

### 0.3.0-beta.2 の既知制限

この修正版固有の新しい既知制限は、公開時点では確認されていません。IME入力、Markdownの正規化、未実装機能などbeta全体の既知制限は、この文書の後半を参照してください。

### 観察期間中に確認してほしいこと

- Nyoze を操作していない状態で renderer の CPU 使用率が高止まりしないこと
- `# 　見出し`のように先頭へ全角空白を含む見出しを保存・再読込しても、空白が増えないこと
- タブ列右端の操作アイコンで tooltip が表示されること
- `0.3.0-beta.1` で追加した Book / Project、Page Viewer、Web Book、外部 export の主要操作に回帰がないこと
...
- エディタのタブ列右端にある一部のアイコンでは、ツールチップが表示領域から隠れ、確認できないことがあります。アイコンは左から **検索**、**ルビ表示**、**段落プレーン編集**、**ソースモード**、**タイプライター**、**表示設定**、**Page Viewer** の順です。各機能自体は利用でき、この問題は次回の修正版で対応予定です。
