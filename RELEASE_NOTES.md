...
この文書を、beta テスター向けの既知制限・配布上の注意・報告時の注意の正本として扱います。現行の GitHub pre-release は `0.3.0-beta.1`、次の修正版は `0.3.0-beta.2` です。
...
- `0.3.0-beta.1` で確認された高 CPU 使用率を修正しているため、GitHub 版の利用者には更新を推奨します。
- 公開後は改めて約1週間の観察期間を置き、重大な不具合、回帰、配布上の問題、小さな polish だけを扱います。
...
`0.3.0-beta.1` 公開後に確認された不具合を修正する pre-release です。Book / Project、Page Viewer、Web Book、外部 export の機能範囲は広げず、安定化と案内の修正だけを行っています。
...
この修正版固有の新しい既知制限は、現時点では確認されていません。IME 入力、Markdown の正規化、未実装機能など beta 全体の既知制限は、この文書の後半を参照してください。
